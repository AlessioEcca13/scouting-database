...
def generate_notes(tm_data: Dict) -> str:
    """Genera note automatiche dai dati Transfermarkt"""
    notes = "Importato da Transfermarkt\n"
    notes += f"Link: {tm_data.get('url', '')}\n"
    notes += f"Club: {tm_data.get('current_club', '')}\n"
    
    if tm_data.get('date_of_birth_text'):
        notes += f"Data di nascita: {tm_data.get('date_of_birth_text')}\n"
    else:
        notes += "Data di nascita: N/A\n"
    
    if tm_data.get('birth_place'):
        notes += f"Luogo di nascita: {tm_data.get('birth_place')}\n"
    
    if tm_data.get('market_value'):
        notes += f"Valore di mercato: {tm_data.get('market_value')}\n"
    
    if tm_data.get('shirt_number'):
        notes += f"Numero maglia: #{tm_data.get('shirt_number')}\n"
...
def map_to_database_format(tm_data: Dict) -> Dict:
    ...
    db_data = {
        ...
        'general_role': map_position_to_role(tm_data.get('position', '')),
        'specific_position': tm_data.get('position', ''),
        ...
        'natural_position': tm_data.get('natural_position', ''),
        'other_positions': tm_data.get('other_positions', ''),
